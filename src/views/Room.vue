<template>
  <div class="room">
    ...
    <Player @add-track="handleTrackAddition" v-bind:currentTrackPlaying="currentTrackPlaying" @skip-track="skipTrack"/>
    <TrackList v-bind:currentTrackData="currentTrackData"/>
    <UserList v-bind:currentUsers="currentUsers"/>
...
export default {
  ...
  data() {
    return {
      ...
      currentUsers: [],
      currentTrackPlaying: {'track': '', 'playing': false},
    ...
    skipTrack() {
      ...
      this.socketConnect.on('trackSkipped', (data) => {
        ...
        if (data.hasOwnProperty('error')) {
          alert(data.error); //# DEV-NOTE Fine a better way to represent this error
        }
  ...
  created() {
    ...
    this.socketConnect.on('disconnected', (data) => {
