...
export default {
  ...
  methods: {
    createRoom: function (roomData) {
      ...
      this.socketConnect.on('serverCreated', (roomData) => {
        LobbyStore.state.rooms = roomData['roomData'];
  ...
  mounted() {
    ...
    this.socketConnect.on('servers', (roomData) => {LobbyStore.state.rooms = roomData;});
    this.socketConnect.on('updateLobby', (data) => {
      // 1. Add to "rooms" array
      LobbyStore.state.rooms.forEach(function(curr, index) {
        // Find the room "name" from data
        let currentData = data.filter(current => current.name === curr.name)[0];
        
        if (currentData !== undefined)
          LobbyStore.state.rooms[index].users = currentData.users;
      }, this);
